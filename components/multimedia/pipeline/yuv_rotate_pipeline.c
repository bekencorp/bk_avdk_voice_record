...
#define SPI_DISPLAY_START()			//do { GPIO_UP(5); } while (0)
#define SPI_DISPLAY_END()			//do { GPIO_DOWN(5); } while (0)
...
#define SPI_DISPLAY_START()
#define SPI_DISPLAY_END()
...
//    partial_area->start_y = PIPELINE_DECODE_LINE * (rotate_config->dma2d_isr_cnt - 1);
    partial_area->start_y = PIPELINE_DECODE_LINE * (rotate_buf->index - 1);

    partial_area->width = rotate_config->lcd_info.width;  //240

    partial_area->height = PIPELINE_DECODE_LINE;
    partial_area->post_refresh = partial_display_complete_cb;

    if (rotate_config->partial_display_start == 0)
    {
        ret = lcd_display_get_info(&rotate_config->lcd_info);
        if (ret == BK_OK)
        {
            if (rotate_buf->index == 1)
            {
                rotate_config->partial_display_start = 1;
            }
            else
            {
    ...
    if (rotate_config->lcd_info.interface == LCD_TYPE_SPI)
    {
        #if CONFIG_LCD_SPI_DISPLAY
        SPI_DISPLAY_START();
        if (rotate_config->rot_mode == HW_ROTATE)
        {
            //swap: 0X01234567 => 0X67452301
            #define BYTE_SWAP32(x) ( ((x >> 24) & 0xFF) | ((x >> 8) & 0xFF00) | ((x << 8) & 0xFF0000) | ((x << 24) & 0xFF000000)) //60us
            uint32_t *buf32 = (uint32_t *)rotate_buf->data;
            uint32_t count = (rotate_config->jpeg_width * partial_area->height) >> 1;
            for (int k = 0; k < count; k++)
            {
                buf32[k] = BYTE_SWAP32(buf32[k]);
            }
            for (int i = 0; i < partial_area->height; i++)  //1.58ms
            {
                lcd_spi_partial_display(rotate_buf->data + (rotate_config->jpeg_width * 2 * i), partial_area->width * 2);
            }
        }
        else
        {
            lcd_spi_partial_display(rotate_buf->data, partial_area->width * partial_area->height * 2);
        }
        SPI_DISPLAY_END();
        partial_display_complete_cb(partial_area);
    ...
    else if (rotate_config->lcd_info.interface == LCD_TYPE_MCU8080)
    {
        ret = lcd_display_partial_request(partial_area);
        if (ret != BK_OK)
        {
            LOGD("%s send display request fail\n", __func__);
            partial_display_complete_cb(partial_area);
        }
    }
    else if (rotate_config->lcd_info.interface == LCD_TYPE_QSPI)
    {
        #if CONFIG_LCD_QSPI
        #if CONFIG_LCD_QSPI_TE
        if (rotate_buf->index == 1 || rotate_buf->index == rotate_config->jpeg_height / PIPELINE_DECODE_LINE / 2 + 1)
        {
            lcd_qspi_te_flag = true;
            ret = lcd_qspi_te_wait(50);
            if (ret != BK_OK)
            {
                LOGE("%s lcd_qspi_te_wait timeout\n", __func__);
                return;
            }
            lcd_qspi_te_flag = false;
            rtos_delay_milliseconds(4);
        }
        #endif
        bk_lcd_qspi_partial_display(0, g_lcd_device, partial_area->start_x, partial_area->start_x + partial_area->width - 1, partial_area->start_y,  partial_area->start_y + partial_area->height - 1, rotate_buf->data);
    ...
    {
        lcd_display_get_info(&rotate_config->lcd_info);
        partial_display_complete_cb(partial_area);
    }
#endif
}
...
//	dma2d_memcpy_pfc.dma2d_width = rotate_config->jpeg_width;
    dma2d_memcpy_pfc.dma2d_width = rotate_config->lcd_info.width;  //240

	dma2d_memcpy_pfc.dma2d_height = PIPELINE_DECODE_LINE;
	dma2d_memcpy_pfc.src_frame_width = rotate_config->jpeg_width;
	dma2d_memcpy_pfc.src_frame_height = PIPELINE_DECODE_LINE;
//	dma2d_memcpy_pfc.dst_frame_width = rotate_config->jpeg_width;
    dma2d_memcpy_pfc.dst_frame_width = rotate_config->lcd_info.width;  //240
...
bk_err_t rotate_task_open(rot_open_t *rot_open)
{
    ...
    if (ret == BK_OK)
    {
        LOGI("%s get lcd info: lcd_type = %d (0/1:rgb,2:mcu,3:qspi,4:spi ), lcd_width * height = (%d * %d)\n", __func__, 
            rotate_config->lcd_info.interface, rotate_config->lcd_info.width, rotate_config->lcd_info.height);
    }
    else
    {
	...
	{
		ret = rtos_init_oneshot_timer(&rotate_timer, 1 * 2000, rotate_timer_handle, NULL, NULL);
