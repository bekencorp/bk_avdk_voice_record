...
static void jpeg_decode_start_handle(frame_buffer_t *jpeg_frame)
{
	...
	if (jdec_config->jdec_init == false)
	{
		...
		else
		{
			...
			msg_send_req_to_media_major_mailbox_sync(EVENT_JPEG_DEC_INIT_NOTIFY, MINOR_MODULE, (uint32_t)(mux_sram_buffer->rotate), NULL);
#else
	...
	else
	{
		if (jdec_config->jdec_type == JPEGDEC_BY_LINE)
		{
			...
			ret = bk_jpeg_dec_sw_start(JPEGDEC_BY_LINE, jdec_config->jpeg_frame->frame, jdec_config->decoder_buf,
						jdec_config->jpeg_frame->length, sizeof(mux_sram_buffer->decoder), &result);
#else
			ret = BK_FAIL;
