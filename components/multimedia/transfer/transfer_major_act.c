...
static void transfer_major_task_transfer_data(uint32_t param)
{
	...
	{
		if (transfer_info.param & IMAGE_MJPEG)
		{
			stream = frame_buffer_list_get_main_stream();
		}
		else
		{
			stream = frame_buffer_list_get_by_format(transfer_info.param);
		}
		...
		if (stream == NULL)
		{
			...
			if (transfer_major_task_running)
			{
		...
		frame_buffer_fb_read_free(transfer_info.stream, encode_frame, MODULE_WIFI);
	}
}
...
static bk_err_t transfer_major_task_open_handle(media_mailbox_msg_t *msg)
{
	...
	transfer_info.param = msg->param;

	ret = transfer_major_task_init();
	if (ret == BK_OK)
	{
		if (transfer_major_task_send_msg(TRS_TRANSFER_DATA, 0) == BK_OK)
		{
			LOGI("%s, transfer format:%d\r\n", __func__, transfer_info.param);
