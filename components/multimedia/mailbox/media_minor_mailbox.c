	...
	if (msg->src == MAJOR_MODULE)
	{
		if(msg->type == MAILBOX_MSG_TYPE_REQ) //send req msg to cpu2 other threads
		{
			switch (msg->event)
			{
				...
				{
					ret = software_decode_task_close();
				}
				break;
				case EVENT_JPEG_DEC_INIT_NOTIFY:
				{
					ret = software_decode_task_open(msg->param);
				}
				break;
				case EVENT_JPEG_DEC_SET_ROTATE_ANGLE_NOTIFY:
				{
					...
					{
						software_decode_set_rotate(rotate_angle);
					}
					else
					{
						ret = BK_ERR_PARAM;
				...
				break;
#endif
				case EVENT_YUV_ROTATE_DEINIT_NOTIFY:
				{
					ret = software_rotate_task_close();
				}
				break;
				case EVENT_YUV_ROTATE_INIT_NOTIFY:
				{
		...
		else if(msg->type == MAILBOX_MSG_TYPE_NOTIFY)
		{
			switch (msg->event)
			{
				...
				{
					software_decode_task_send_msg(msg->event, (uint32_t)msg);
				}
#endif
				break;
				case EVENT_YUV_ROTATE_START_NOTIFY:
				{
					software_rotate_task_send_msg(msg->event, (uint32_t)msg);
				}
				break;
				default:
					break;
...
static void media_minor_mailbox_message_handle(void)
{
	...
	asr_to_media_major_msg.event = EVENT_AUD_ASR_INIT_NOTIFY;
