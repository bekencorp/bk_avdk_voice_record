...
static void lcd_spi_dma_init(const lcd_device_t *device)
{
    bk_err_t ret = BK_OK;
...
void lcd_spi_set_display_area(uint16_t x_start, uint16_t x_end, uint16_t y_start, uint16_t y_end)
{
    uint8_t column_value[4] = {0};
    uint8_t row_value[4] = {0};
...
bk_err_t lcd_spi_partial_display(uint8_t *disp_buffer, uint32_t data_len)
{
    if (lcd_spi_open_status == false)
    {
        return BK_FAIL;
    ...
    lcd_spi_send_data(disp_buffer, data_len);
